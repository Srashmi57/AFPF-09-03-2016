...
class FileuploadComponent extends Component 
{
    ...
    function __construct($options = null, $initialize = false, $error_messages = null) {
		...
		$this->options = array(
            ...
            'accept_file_types' => '/.(gif|jpe?g|png|pdf|mp3|mp4|mov|docx|pdf|m4v|f4v|m4a|aac|f4a|WebM|ogg)+$/i',
            // The php.ini settings upload_max_filesize and post_max_size
            // take precedence over the following max_file_size setting: 'max_file_size' => 50000000
            'max_file_size' => 50000000000000000000000,
            'min_file_size' => 1,
            // The maximum number of files for the upload directory:
            'max_number_of_files' => 5000,
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
			...
			else
			{
				...
				{*/
					...
					//exit;
					//$fh = fopen("checkdet","w");
					//$str = "upload_max_size - ".ini_get('upload_max_filesize');
					//$str .= ",post_max_size".ini_get('post_max_size');
					//$str .= ",max_input_time".ini_get('max_input_time');
...
{
			$current_user_id = CakeSession::read('Auth.WebsitesUser.user_id');
			$usertype_id = CakeSession::read('Auth.WebsitesUser.usertype_id');
		...
		{
			 $modelusermediacat = ClassRegistry::init('usercategories');
			 $catDetail = $modelusermediacat->find('all',array('fields' =>array('category_id'),'conditions'=>array('user_id'=>$current_user_id)));
			 foreach($catDetail as $cats)
			 {
		
		        $catarray[] = $cats['usercategories']['category_id'];
				
		
			 }
			 //$condition = array('IN' => array('category.category_id'=>$catarray));
			 $condition = array('UserMedia.category_id' => $catarray);
		}
...
public function getmyrecentuploadsbyselected()
{
			$current_user_id = CakeSession::read('Auth.WebsitesUser.user_id');
			$usertype_id = CakeSession::read('Auth.WebsitesUser.usertype_id');
		...
		{
			 $modelusermediacat = ClassRegistry::init('usercategories');
			 $usersubcategories = ClassRegistry::init('usersubcategories');
			 $usersubsubcategories = ClassRegistry::init('usersubsubcategories');
			 $catDetail = $modelusermediacat->find('all',array('fields' =>array('category_id'),'conditions'=>array('user_id'=>$current_user_id)));
			 foreach($catDetail as $cats)
			 {
		        $catid = $cats['usercategories']['category_id'];
		        $catarray[] = $cats['usercategories']['category_id'];
			 ...
			 $catsubDetail = $usersubcategories->find('all',array('fields' =>array('subcategory_id'),'conditions'=>array('user_id'=>$current_user_id)));
			 foreach($catsubDetail as $subcats)
			 {
		
		        $subcatarray[] = $subcats['usersubcategories']['subcategory_id'];
				
		
			 }
			 
			 if(sizeof($catsubDetail) > 0){
				 $condition = array('UserMedia.category_id' => $catarray,'UserMedia.subcategory_id' => $subcatarray);
				 
			 }
			 
			 
			 $catsubsubDetail = $usersubsubcategories->find('all',array('fields' =>array('subsubcategory_id'),'conditions'=>array('user_id'=>$current_user_id)));
			 foreach($catsubsubDetail as $subsubcats)
			 {
		
		        $subsubcatarray[] = $subsubcats['usersubsubcategories']['subsubcategory_id'];
				
		
			 }
			 
			 if(sizeof($catsubsubDetail) > 0 && sizeof($catsubDetail) > 0){
				  $condition = array('UserMedia.category_id' => $catarray,'UserMedia.subcategory_id' => $subcatarray,'UserMedia.subsubcategory_id' => $subsubcatarray);
				  
			 }
			 
			 
			 
			 /* get all categoary values */
			    $arr_CategoryList = array();
				$modelCategory = ClassRegistry::init('Category');
				$arr_CategoryList = $modelCategory->find('list',array('fields'=>array('category_id','category_name'),'conditions'=>array('category_status'=>1),'order'=>array('category_order'=>'asc')));
				
				
			 /* end here */
			 foreach($arr_CategoryList as $usercatkey=> $usercat) {
				 
				if(in_array($usercatkey,$catarray))
				{
					
					  $modelsubCategory = ClassRegistry::init('Subcategory');
			          $arr_subsubCategoryList =  $modelsubCategory->find('list',array(
																	'fields'=>array('subcategory_id',
																					'subcategory_name'),
																	'conditions'=>array('category_id'=>$usercatkey, 'subcategory_status'=>1)
																)
													);
					
					if(sizeof($arr_subsubCategoryList) > 0){
						foreach($arr_subsubCategoryList as $usersubcatkey=> $usersubcat)
						{
							 
							if(in_array($usersubcatkey,$subcatarray))
							{
								 $pstring .= "(UserMedia.category_id='$usercatkey' && UserMedia.subcategory_id='$usersubcatkey')||";
								 //echo $usersubcat;echo '<br>';
								 ...
								 $arr_subsubCategoriesList = $modelSubsubcategory->find('list',array(
																	'fields'=>array('subsubcategory_id',
																					'subsubcategory_name'),
																	'conditions'=>array('subcategory_id'=>$usersubcatkey, 'subsubcategory_status'=>1)
																)
													);
													
								 if(sizeof($arr_subsubCategoriesList) > 0){
									 foreach($arr_subsubCategoriesList as $usersubsubcatkey=> $usersubsubcat)
									 {
										 if(in_array($usersubsubcatkey,$subsubcatarray))
										 {
										      
											  $pstring .= "(UserMedia.category_id='$usercatkey' && UserMedia.subcategory_id='$usersubcatkey' && UserMedia.subsubcategory_id='$usersubsubcatkey')||";
											//echo $usersubsubcatkey;
										 } else { $getUnchecksubsubcatlist[] = $usersubsubcatkey; }
									 }
								 }else{
								  $pstring .= "(UserMedia.category_id='$usercatkey' && UserMedia.subcategory_id='$usersubcatkey')||";
								 }
							}else{
							   $getUnchecksubcatlist[] = $usersubcatkey;
							}
						}
					}else{
						$pstring .= "(UserMedia.category_id='$usercatkey')||";
					}
		...
		//echo $pstring;
		$arrmediaDetail007 = $modelusermedia->getrecentuploadbyselected($pstring);
		
		$arrmediaDetail = $modelusermedia->find('all',array( 
   'fields' => array('UserMedia.*','users.user_fname','users.user_lname','users.user_id','users.user_display_name','cat.category_name','subcat.subcategory_name','subsubcat.subsubcategory_name'),
  'joins' => array(
        array(
            'table' => 'users',
            'alias' => 'users',
	...
	if(count($arrmediaDetail007))
	{

	
		foreach($arrmediaDetail007 as $mediakey=>$medialist)
		{
			 $retcover_id = $medialist['UserMedia']['cover_id'];
			...
			if($retcover_id>0)
			{
				$arrmediaDetail007[$mediakey]['children'] = $modelusermedia->find('first',array('fields' => array('UserMedia.usermedia_title','UserMedia.usermedia_path'),'conditions'=>array('usermedia_id'=>$retcover_id)));
				
			}
			else
			{
				$arrmediaDetail007[$mediakey]['children']= array();
			}
			
			$subsubstr = implode(',',$getUnchecksubsubcatlist);
			$substr = implode(',',$getUnchecksubcatlist);
		    $arrmediaDetail007[$mediakey]['subsubuncheckvalue'] = $subsubstr;
			$arrmediaDetail007[$mediakey]['subuncheckvalue'] = $substr;
		...
		/* if(in_array($usersubsubcatkey,$getUnchecksubsubcatlist))
		{
		} */
		
		return $arrmediaDetail007;	
}
...
public function getCheckRecentMedia()
	{
		...
		//$arrmediaDetail = $modelusermedia->find('all',array('conditions'=>array('NOT' => array('user_id'=>$current_user_id)),'order' => 'usermedia_date desc','limit' => 8));
		 $strQuery="Select UserMedia . * , users.user_fname, users.user_lname, users.user_display_name, users.user_id, category.category_name, category.category_id, subsubcategory.subsubcategory_name,subcategory.subcategory_name from appap14_usermedia as UserMedia inner join appap14_users  as users  on users.user_id = UserMedia.user_id inner join appap14_category as category on  UserMedia.category_id = category.category_id 
		 left join appap14_subsubcategory as subsubcategory on UserMedia.subsubcategory_id = subsubcategory.subsubcategory_id left join appap14_subcategory as subcategory on UserMedia.subcategory_id = subcategory.subcategory_id
...
public function fnaverageratedmedia007($catname,$categoryid,$artist_id)
{
	$modelusermedia = ClassRegistry::init('UserMedia');
	$condition = "";
	switch($catname)
	{
		...
		case 'subsubcategory':
				 ...
				 break;
		case '':
		         $condition = "UserMedia.user_id =".$artist_id;
				
				 break;	 				 
		
	}
	$arrmediaDetail =	$modelusermedia->topuseraveragemedia007($condition);
	/*  echo "<pre>";
	print_r($arrmediaDetail);
	exit(); */
