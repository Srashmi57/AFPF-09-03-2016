...
<?php 
	...
	//subsubcategory_id
    $p=0;	
   $subsubcheckarry = explode(',',$arrrecentuploadsbyselected[0]['subsubuncheckvalue']);
   $subcheckarry = explode(',',$arrrecentuploadsbyselected[0]['subuncheckvalue']);
   /* echo '<pre>';
   print_r($subsubcheckarry);
   echo '</pre>'; */
	foreach($arrrecentuploadsbyselected as $mediarecent )	
	{	
	   
	    if(!in_array($mediarecent['UserMedia']['subsubcategory_id'],$subsubcheckarry) && !in_array($mediarecent['UserMedia']['subcategory_id'],$subcheckarry))
		{
	    ...
	    if($MediaRating[$p][0]['MediaRating']['usermedia_id']==$umedia_id){
	     ...
	     $count=0;
								...
								$isloggedclass = $islogin>0?'':"disabled='disabled'";
									 if(isset($mediarecent['subcat']['subcategory_name']))
										{
											$category_name = $mediarecent['subcat']['subcategory_name'];
										
										}
										else if(isset($mediarecent['subsubcat']['subsubcategory_name']))
										{
											$category_name = $mediarecent['subsubcat']['subsubcategory_name'];
										}
										else
										{
											$category_name = $mediarecent['cat']['category_name'];
										}
										$cmtclass = $islogin>0?'':"display:none";
										$retuser_id = $mediarecent['UserMedia']['user_id'];
										if($user_id==$islogin)
										{
		...
		$categoryname = $mediarecent['cat']['category_name'];
		$subcategoryname = $mediarecent['subcat']['subcategory_name'];
		$susubcategoryname = $mediarecent['subsubcat']['subsubcategory_name'];
		 ...
		 <div class="recent_cat_star">
			...
			<div class="gallerytext">
				<div class="medianame" id="getpos_<?php echo $usermedia_id;?>">
					<?php 
						echo $this->Html->link(mb_strimwidth($usermedia_name,0, 12, ".."), array('controller'=>'users','action' => "art", $usermedia_id), array( 'id'=>'msg-name', 'escape' => false)); ?> <div class="pull-right media-name"> <?php echo $categoryname;?> <?php if($susubcategoryname!=""){echo " - <span class='subsubcat_name'>".$susubcategoryname."</sapn>";}else{{echo " - <span class='subsubcat_name'>".$subcategoryname."</sapn>";}}?> </div> &nbsp;
